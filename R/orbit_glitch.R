...
orbit_glitch <- function(seed_probs = c(1, 0, 0, 0), glitch_type = "spike", pareto2_prob = c(0.2, 0.8),
  ...
  if(pareto_2) {
    ...
    pareto_orbits2 <- data.frame(x = 0, y = 0, id = 0, linewidth = 0, linetype = "solid")
  }
  ...
  plot <-
    ggplot() +
    geom_polygon(data = final_dat[["seed"]], aes(x = x, y = y)) +
    geom_path(data = final_dat[["seed_outlines"]], aes(x = x, y = y)) +
    geom_path(data = final_dat[["pareto1"]], aes(x = x, y = y)) +
    geom_path(data = final_dat[["pareto2"]], aes(x = x, y = y)) +
    geom_path(data = final_dat[["orbits"]], aes(x = x, y = y)) +
    scale_size_identity() +
